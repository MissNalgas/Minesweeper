...
namespace Weasel {
	...
	static uint32_t ShaderDataTypeSize(ShaderDataType type)
	{
		...
		WS_CORE_ASSERT(false, "Unknown ShaderDataType!");
		return 0;
	}
	...
	struct BufferElements
	{
		...
		BufferElements(const std::string& name, ShaderDataType type, bool normalized = false) : Name(name), Type(type), Size(ShaderDataTypeSize(type)), Offset(0), Normalized(normalized) {}
	...
	//Supports 32-bit index buffer only
	class IndexBuffer
	{
	public:
		virtual ~IndexBuffer() {}
